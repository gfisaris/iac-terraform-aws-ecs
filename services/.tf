resource "aws_ecs_service" "" {
  name = ""

  cluster = "${aws_ecs_cluster.master.id}"
  task_definition = "${aws_ecs_task_definition.AWS_ECS_TASK_DEFINITION.arn}"

  desired_count = 9
  deployment_maximum_percent = "200"
  deployment_minimum_healthy_percent = "100"

  iam_role = "${data.terraform_remote_state.aws_iam.iamRole_ecsServiceSchedulerRole}" 

  load_balancer {
    target_group_arn = "${data.terraform_remote_state.aws_ec2_alb.tgrp--arn}"
    container_name = "nginx"
    container_port = "80"
  }
}

output "Service--ID" {
  value = ["${aws_ecs_service..id}"]
}

output "Service--Name" {
  value = ["${aws_ecs_service..name}"]
}

output "Service--Cluster" {
  value = ["${aws_ecs_service..cluster}"]
}

output "Service--IamRole" {
  value = ["${aws_ecs_service..iam_role}"]
}

output "Service--DesireCount" {
  value = ["${aws_ecs_service..desire_count}"]
}

